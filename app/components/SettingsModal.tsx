    ...
    try {
      ...
      if (response.ok) {
        ...
        const newSettings = {
          ...
          scraper_timeout: parseInt(data.settings.scraper_timeout) || 60000,
          scraper_log_http_requests: parseBool(data.settings.scraper_log_http_requests),
  ...
  const handleDownloadBackup = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.error('Backup export failed', new Error(`HTTP ${response.status}`));
      }
    } catch (error) {
  ...
  const handleDeleteAllTransactions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setDeleteResult({
          success: true,
          message: data.message || `Successfully deleted ${data.deletedCount} transactions`
        });
            ...
            <>
              ...
              <SettingSection>
                ...
                <SettingRow>
                  ...
                  <Select
                    value={settings.fetch_categories_from_scrapers ? 'always' : 'never'}
                    onChange={(e) => {
                ...
                <SettingRow>
                  <Box>
                    ...
                    <Typography variant="caption" sx={{ color: theme.palette.text.secondary }}>
                      Global timeout for all vendors (default: 60000ms)
                    </Typography>
                  </Box>
                  <StyledTextField
                    type="number"
                ...
                <SettingRow>
                  <Box>
                    ...
                    <Typography variant="caption" sx={{ color: theme.palette.text.secondary }}>
                      Log all HTTP requests made by the scraper (for debugging rate limiting)
                    </Typography>
                  </Box>
                  <Switch
              ...
              <SettingSection>
                ...
                <SettingRow>
                  <Box>
                    <Typography variant="body1">Delete All Transactions</Typography>
                    <Typography variant="caption" sx={{ color: theme.palette.text.secondary }}>
      ...
      >
        ...
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 3 }} icon={<WarningAmberIcon />}>
            This action will permanently delete all transactions from your database. This cannot be undone!
          </Alert>

          <Box sx={{ mb: 3 }}>
            <Typography variant="body1" sx={{ mb: 2, fontWeight: 500 }}>
              Before deleting, we recommend downloading a backup:
            </Typography>
            <Button
              variant="outlined"
          ...
          {deleteResult && (
            <Alert
              ...
              sx={{ mt: 2 }}
              icon={deleteResult.success ? <CheckCircleIcon /> : <ErrorIcon />}
            >
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          ...
          <Button
            ...
            startIcon={deleting ? <CircularProgress size={20} color="inherit" /> : <DeleteOutlineIcon />}
            sx={{
