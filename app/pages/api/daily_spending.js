...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (cycle) {
      // Billing cycle mode - 10th of previous month to 9th of selected month
      ...
      // Calculate billing cycle dates: 10th of previous month to 9th of selected month
      ...
      periodStartDate = `${prevYear}-${String(prevMonth).padStart(2, '0')}-10`;
      periodEndDate = `${year}-${String(month).padStart(2, '0')}-09`;
      cycleLabel = cycle;
    } else {
    ...
    const [dailyResult, budgetResult] = await Promise.all([
      client.query(dailySpendingSql, [periodStartDate, periodEndDate]),
