...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Also update any categorization rules that target these categories
    await client.query(
      `UPDATE categorization_rules 
       SET target_category = $1, updated_at = CURRENT_TIMESTAMP
       WHERE target_category = ANY($2)`,
      [trimmedNewName, sourceCategories]
    );

    // Store the mappings so future scrapes can use them
    for (const source of sourceCategories) {
      if (source === trimmedNewName) continue; // Don't map to itself
