 ...
 * Nudlers MCP Server
 * 
 * This MCP server exposes the Nudlers personal finance application's
 * functionality to AI assistants like Claude Desktop or Cursor.
 * 
 * Requirements:
 * - Nudlers app must be running (default: http://localhost:6969)
 * - Set NUDLERS_API_URL environment variable if using a different URL
 */
...
// Configuration
const API_BASE = process.env.NUDLERS_API_URL || "http://localhost:6969";
...
server.tool(
    ...
    async ({ billingCycle, startDate, endDate, groupBy }) => {
        try {
            ...
            const data = await apiRequest<any[]>(`/monthly_summary?${params}`);
...
server.tool(
    ...
    async ({ category, billingCycle, startDate, endDate }) => {
        try {
            ...
            const data = await apiRequest<any[]>(`/category_expenses?${params}`);
...
server.tool(
    ...
    async () => {
        try {
            const data = await apiRequest<string[]>("/get_all_categories");
...
server.tool(
    ...
    async ({ query, billingCycle, startDate, endDate }) => {
        try {
            ...
            const data = await apiRequest<any[]>(`/search_transactions?${params}`);
...
server.tool(
    ...
    async ({ billingCycle }) => {
        try {
            ...
            const data = await apiRequest<any>(`/budget_vs_actual?${params}`);
...
server.tool(
    ...
    async () => {
        try {
            const data = await apiRequest<any>("/sync_status");
...
// ============================================================================
// TOOL: Add Manual Transaction
// ============================================================================
server.tool(
    "add_manual_transaction",
    "Add a manual expense or income transaction.",
    {
        name: z.string().describe("Transaction description/name"),
        amount: z.number().positive().describe("Transaction amount (positive number)"),
        type: z.enum(["expense", "income"]).describe("Transaction type"),
        category: z.string().optional().describe("Category name (optional for expenses)"),
        date: z.string().optional().describe("Transaction date in YYYY-MM-DD format (defaults to today)"),
    },
    async ({ name, amount, type, category, date }) => {
        try {
            const transactionDate = date || new Date().toISOString().split("T")[0];

            const body = {
                name,
                price: type === "expense" ? -Math.abs(amount) : Math.abs(amount),
                category: category || (type === "income" ? "Income" : "Uncategorized"),
                date: transactionDate,
                isManual: true,
            };

            await apiRequest("/manual_transaction", {
                method: "POST",
            ...
            return {
                content: [{ type: "text", text: `Error adding transaction: ${error}` }],
            };
        }
    }
);
...
server.tool(
    ...
    async () => {
        try {
            const data = await apiRequest<any>("/recurring_payments");
...
async function main() {
    ...
    console.error("Nudlers MCP Server running on stdio");
}
